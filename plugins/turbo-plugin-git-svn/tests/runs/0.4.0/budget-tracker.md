# Budget Tracker — Script tests + Skill tests (v1.0.0 PR validation)

turbo-plugin v1.0.0 release PR-readiness 手動測試 Script tests(orchestrator wall time hr)
與 Skill tests(使用者 session 計數)的 budget 累計與 escalation trigger 紀錄。Template
來源見 [`../../docs/budget-tracker-template.md`](../../docs/budget-tracker-template.md)。
...
>
> 相關:[`../../docs/fail-then-fix-process.md`](../../docs/fail-then-fix-process.md) — F5 流程 + R32 / suspension trigger 細節。
...
- [ ] **同 case fix > 3 次仍 FAIL** → mark `FAIL-known` 在對應 tracking doc + Known
  ...
  [`fail-then-fix-process.md`](../../docs/fail-then-fix-process.md) §R32 Escalation)
- [ ] **連續 3 次 fix 觸到 shared code**(`common.ps1` / `common.sh` /
  ...
  [`fail-then-fix-process.md`](../../docs/fail-then-fix-process.md) §Suspension trigger)
