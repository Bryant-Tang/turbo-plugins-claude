...
function New-Sandbox {
    ...
    # Short stamp + short prefix keep the per-case sandbox within Windows MAX_PATH: the
    # nested .turbo-plugin/worktrees/remote-svn-main bridge worktree adds ~40 chars and a
    # linked worktree's $GIT_DIR aborts with "$GIT_DIR too big" once the path nears 260.
    $stamp = [Guid]::NewGuid().ToString('N').Substring(0, 8)
    $sb = [System.IO.Path]::Combine($script:TpSandboxBase, "tp-$Tag-$stamp")
...
function New-GitMainRepo {
    ...
    # (The full tp-setup gitignore wiring is handled separately; this is the fixture-side companion.)
