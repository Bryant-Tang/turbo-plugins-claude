...
2. **必須**把 log 的每一筆 entry 完整 echo 到對話訊息中,用 markdown code block 包起來,讓使用者直接讀到 log 內容。script 已排成「區塊」格式(每筆用 50 個 `═` 雙線上下框住〔也是筆間分隔〕、內部區段用 49 個 `─` 單線分隔;`變更:` 段只在 `--verbose` 且有變更檔時出現):

   ```
   ══════════════════════════════════════════════════
   r5 | bryant | 2026-05-26T12:34:56.000000Z
   ─────────────────────────────────────────────────
   修正中文檔名
   ─────────────────────────────────────────────────
   變更:
   M  /main/README.md
   ══════════════════════════════════════════════════
   r4 | alice | 2026-05-25T09:00:00.000000Z
   ─────────────────────────────────────────────────
   another commit
   ══════════════════════════════════════════════════
   ```
