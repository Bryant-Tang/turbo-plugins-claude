...
3. **把預覽結果原樣講給使用者聽**：會標記幾個檔、跳過幾個二進位檔、跳過幾個行尾混雜的檔，
   ...
   - **行尾混雜的檔案要不要統一**（只在預覽有列出來時問）。這件事**在主 worktree 就做得到**，
     但它會改到檔案內容，而且要先走完一次推送才輪得到遷移，所以**不要順手做掉**，講清楚是
     哪幾個檔、以及會被改成什麼樣子，讓使用者決定。答「要」就照 Decision Rules 那段的四個
     步驟走；答「不要」就照舊把它們排除、維持現有行尾（**這一輪的遷移照跑，不必等**）。
...
- **行尾混雜的檔案會被這一輪排除，而且要講清楚。** 設了 `svn:eol-style` 之後，SVN 會**拒絕
  commit** 行尾混雜的檔案（錯誤碼 `E135000`）。而 SVN 的 commit 是不可分割的——漏掉一個就
  整批失敗。這些檔案因此被排除、維持它們現有的行尾。這件事事後在任何地方都看不出來，所以
  預覽階段一定要講。

- **要涵蓋它們，就在主 worktree 正規化再推一次，不要去動 bridge。** 四個步驟：①在**主
  worktree**把每個檔案改成單一行尾 ②`git commit` ③`/tp-push-to-svn` ④重跑這支遷移，那些檔案
  這時就會被算成候選。

  **為什麼在主 worktree 有效**：這些檔案是**從 SVN 經 bridge** 進到 git 的，而 bridge 釘 LF、
  不做任何轉換，所以原始的混雜位元組就是 blob 本身；main 靠 merge 拿到的是**同一顆 blob**。
  改了就有真的 diff 可以 commit，普通推送路徑送得上去。實測兩種 `core.autocrlf` 設定結果相同。

  **不要改成在 bridge 上動內容。** 那會讓遷移送出一筆帶內容的 SVN 修訂，而它現在能安全地做
  「一筆沒有 git 對應的提交」，靠的正是那筆**只含屬性**——純屬性的修訂會被拉取路徑跳過。既然
  主 worktree 這條路走得通，就沒有理由去動那個論證。
