...
4. 依 summary 分別處理:
   ...
   - 有 `Skipped (checked out elsewhere)` 分支(**不影響 exit code**)→ **不要講成衝突,也不要叫使用者去比 diff**。每支的 `SKIP <b> (checked out at <path>)` 那行有佔用它的 worktree 絕對路徑,把路徑講給使用者,並說明兩條處置:到那個 worktree 裡直接 `git merge --no-ff main`,或先移除該 worktree 再重跑本 skill。
