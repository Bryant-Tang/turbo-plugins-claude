...
另一條是**煞車**:使用者說要收工、暫停、今天先到這裡時，**停止跨 session 通訊**。理由不是禮貌——peer 之間的往返會出現在使用者的畫面上，即使收件人不是他，所以「收工」的意思包含「不要再產生需要他讀的東西」。要交代的事一次講完然後停，對方回了也不必再回，除非它回報的是會影響正確性的問題。教 agent 主動去找別的 session 卻不教它什麼時候停，等於只給了油門。
...
要涵蓋混雜的那些檔案：在**主 worktree** 把每個檔案改成單一行尾、commit、`/tp-push-to-svn`，
再重跑一次遷移。**這裡有真的變更可以 commit**，因為這些檔案是從 SVN 經 bridge 進到 git 的，
而 bridge 釘 LF、不做任何轉換——原始的混雜位元組就是 blob 本身，main 靠 merge 拿到的是**同一顆
blob**。（若是在主 worktree 新建的檔案就不是這樣：`core.autocrlf=true` 在 `git add` 當下就把
CRLF 清成 LF，blob 天生純 LF。那是檔案**怎麼進來的**造成的差別，不是這個系統的行為。）
...
而且是**在問「乾不乾淨」之前**讀。這個順序是 0.8.0 的教訓：遷移跑完沒有重讀模式，於是
pull、push、連重跑遷移本身都被自己的守門擋住，三個訊息都指著使用者沒做過的變更。模式不對的
時候，「bridge 是髒的」這個判斷根本沒有意義——每個檔案都讀成已修改，跟真的有待處理工作分不
出來。現在守門之前一定先讀，所以那個狀態即使已經存在，下一次 pull 或 push 就自己恢復了。
...
**`/tp-setup` 把專案接到一個全新的 SVN 樹時，會直接把 `svn:eol-style` 宣告下去**（跟
`svn:ignore=.git` 同一筆提交），所以那種 repo **永遠不需要跑遷移**。分支不用另外處理——`svn copy`
會把宣告帶過去，分支底下新增的檔案自動拿到 `svn:eol-style=native`。

為什麼只對全新的樹這樣做：一棵**已經有檔案**的樹，裡面可能存著 CRLF，要不要動它是需要看過清單再
決定的事——那正是 `/tp-init-svn-eol-style` 帶預覽、且只在你明確要求時才跑的原因。全新的樹沒有這個
問題，所以不必問。

值得知道的差別是**誰在保證**：未宣告的樹靠的是 **bridge 那側的 git 釘選**（bridge 被釘成 LF，所以
交給 svn 的就是 LF），而那只在「所有人都經由這個 plugin 提交」時成立;宣告之後靠的是 **SVN 自己的
屬性**，別的客戶端（例如 TortoiseSVN）推 CRLF 上來一樣會被正規化。

### SVN 有時會把純文字檔當成 binary，而那會擋下推送

`svn add` 用自己的內容啟發式決定一個檔案是不是二進位：跳過 UTF-8 BOM 之後看**前 1024 個位元組**，
出現 NUL 就是 binary，否則只要「文字字元」（`0x07`–`0x0D` 或 `0x20`–`0x7F`）**不足約 15%** 也是 binary。
判成 binary 的檔案會被蓋上 `svn:mime-type=application/octet-stream`，而**帶著那個屬性的檔案設不了
`svn:eol-style`**。

問題在於 **UTF-8 的中文每個位元組都不算文字字元**。所以「散文式、少標題符號、沒有英文術語或程式碼區塊」
的中文文件很容易越線——在一個約兩萬檔的實際 repo 上只有兩支 `.md` 中招，而**哪兩支完全無法預期**，
這正是它難被發現的原因。門檻很硬：1024 個位元組裡有 153 個文字字元判文字、152 個就判 binary。

這件事**在 git 和 svn 兩邊事後都看不出來**：git 一直認為那是文字檔，svn 也不會再重算，那個屬性是
它加進版本庫的當下蓋上去的、之後就一直躺在那裡。直到某次推送剛好動到它，才會在**準備寫入 SVN 的
那一刻**被擋下來。

所以 `/tp-push-to-svn` 在**送出前**就把這些檔案列進確認畫面，並問你要不要把那個標記拿掉。
拿掉之後 SVN 就會把它當文字檔處理（行尾正規化會生效）。**預設不會自動幫你拿掉**——那等於替你斷定
一個檔案不是二進位檔，而 git 的判斷也不是永遠對的。
