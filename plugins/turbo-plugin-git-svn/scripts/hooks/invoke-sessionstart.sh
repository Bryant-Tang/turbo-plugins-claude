...
# On other platforms: run a slim native bash version (marker-missing setup prompt).
set -uo pipefail
...
if [[ -d "$MARKER_DIR" ]]; then
  # Marker present — bootstrap done. turbo-plugin-git-svn has no marker-present runtime
  # concern (dbhub / IIS applicationhost live in sibling plugins), so silent exit.
  emit_json
  exit 0
fi
