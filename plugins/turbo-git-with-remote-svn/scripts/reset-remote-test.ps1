...
try {
    ...
    # Pre-flight: remote-test-<n> worktree clean (git side)
    $remoteStatus = (& git -C $remoteWorktreePath status --porcelain | Out-String).Trim()
    if ($remoteStatus) {
        throw "Remote test worktree '$remoteWorktreePath' has uncommitted changes. Run /tgs:push-to-svn or /tgs:pull-from-svn to resolve first.`n$remoteStatus"
    }

    # Compute and emit diff: lines starting with LOSE (commits dropped) and GAIN (commits brought in)
    ...
    # Diff was just informational; proceed to actual reset.
    ...
    Write-Output ''
    Write-Output "Reset $testBranch to main. Run /tgs:push-to-svn --branch $testBranch to publish."
}
catch {
