...
1. 把 `default-files/.turbo-plugin/dbhub.example.local.toml` 放到專案根的 `.turbo-plugin/dbhub.example.local.toml`（committed 範本），再複製成 `.turbo-plugin/dbhub.local.toml`（gitignored）填入實際連線字串。
2. 需要本機有 Docker（`tp-dbhub` 以 `docker run bytebase/dbhub` 啟動）。

> `.turbo-plugin/` 為四個 turbo-plugin 共用的專案根設定目錄；本 plugin 只寫自己的 db 相關檔。
