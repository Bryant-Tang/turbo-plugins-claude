...
3. **專案根 `.gitignore`** — 確保含 base 標記區塊,內容如下。**用上面「更新自己區塊的通用程序」處理**
   ...
   # <<< turbo-plugin:base <<<
   ```
   - **寫這個區塊之前先決定 `/TODO.md` 那一行要不要放**:跑 `git ls-files --error-unmatch TODO.md`,
     **回 0(已被追蹤)就把那一行拿掉**,其餘照上面寫。理由與第 4 項相同——那代表這個專案刻意把
     `TODO.md` 進版控,加 ignore 不會讓它消失(gitignore 對已追蹤檔無效),只會讓後續每個人都困惑。
     這個判斷必須在**這裡**做,不能留到第 4 項:區塊一旦寫下去就已經含那一行了。
...
4. **專案根 `TODO.md`** — 不存在則建立下面的骨架;**已存在則完全不碰**(裡面是使用者的內容)。
   ...
   - **骨架自己講明定位**,理由同上面的 Marker 慣例:一個「不進版控卻要交接」的檔案很反直覺,
     不寫在檔案裡就等於沒人知道。
   - **`TODO.md` 已被 git 追蹤時,不要動那個檔**(ignore 那一行在第 3 項就已經依同一個判斷略過了):
     那代表這個專案刻意把它進版控,這裡的假設不成立。照實告訴使用者,讓他決定。
