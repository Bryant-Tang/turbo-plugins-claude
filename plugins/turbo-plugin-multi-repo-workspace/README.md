...
### 在**子專案裡**開隔離,會自動改建成工作區鏡像（issue #106）

先 `cd` 進某個子專案(讀程式碼時最自然的動作),再呼叫 `EnterWorktree` —— 這時當前目錄是一個**普通
git repo**,hook 照樣會被呼叫,但走的是「一般 repo」那條路,於是只隔離**那一個專案**。

那個失敗是**完全靜默的**:不會報錯,編輯、建置、commit 全部正常,唯一的差別是這個 session 從此
動不了其他子專案 —— 而這件事要等到真的需要動第二個專案時才會發現,那時改動已經落在錯的地方。

所以 hook 會偵測「當前 repo 是某個**已宣告**工作區的直屬子專案」,**改成隔離整個工作區**,並在
stderr 說明發生了什麼:

```
'<工作區根>/proj-1' is a project inside the multi-repo workspace '<工作區根>'.
Isolating the WHOLE workspace instead: ...
Set TP_WORKTREE_SCOPE=project to isolate just this repository.
```

**為什麼是自動改建而不是直接擋下來**:這個判斷在被逼問的當下**答不出來** —— 沒有人知道等一下會不會
需要動第二個專案。而兩種猜錯的代價不對等:隔離整個工作區只是多開幾份 worktree;只隔離一個專案,
是在**已經有未提交改動**之後才撞到的死路。

**真的只想要這一個專案**:設 `TP_WORKTREE_SCOPE=project`。無法辨識的值(打錯字)會**退回安全行為**
並在 stderr 說明,不會靜默選到沒有保護的那一邊。

> 兩種**不受影響**的情況:①不屬於任何工作區的一般 repo,行為完全不變;②從**既有鏡像裡**的專案再開
> 一份 —— 鏡像目錄沒有工作區標記,所以那裡是普通 repo,不會遞迴展開。
