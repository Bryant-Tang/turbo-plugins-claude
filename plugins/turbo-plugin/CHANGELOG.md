...
- test: `/ce-code-review` 第三輪自審——round-trip 測試再硬化:失敗傳播測試補 `_load_svn_status_xml` 載入守衛(防 sed 抽取失敗時呼叫未定義函式回 127、`assertNotEquals` 假綠);新增 `--` option terminator 結構守衛 `test_submit_scripts_keep_option_terminator`(同時 grep `submit-svn-commit.sh` 與 `Submit-SvnCommit.ps1` 的 svn add/delete/commit,任何環境都跑,補上 leading-dash 行為測試在 Windows / hostile console 下 env-gated skip 後的跨平台缺口);`Svn-StatusXml-Roundtrip.test.ps1` 的 code-point 註解加「勿還原中文字面、本檔須維持純 ASCII 無 BOM」警告
