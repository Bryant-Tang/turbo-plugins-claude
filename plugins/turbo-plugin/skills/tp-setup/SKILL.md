...
5. `.gitignore` 缺 turbo-plugin patterns → idempotent append（patterns 含 `.claude/**/*.local.*`、`.turbo-plugin/**/*.local.*`、`.turbo-plugin/worktrees/`;後者確保任何 `tp-create-remote-test` 建立的 nested worktree 容器不污染主 worktree `git status`）。
