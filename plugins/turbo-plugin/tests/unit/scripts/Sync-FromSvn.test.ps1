...
if (-not [System.IO.File]::Exists($dumpPath)) {
    ...
    try {
        ...
        $resetOut = [System.IO.Path]::Combine([System.IO.Path]::Combine($pluginRoot, 'tests', '.sandbox', 'sandboxes'), "turbo-plugin-reset-out-$([Guid]::NewGuid().ToString('N').Substring(0,10)).txt")
