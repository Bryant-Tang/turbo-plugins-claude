...
#   - format_iis_express_site_name — ASCII + CJK stem
...
# ─── format_iis_express_site_name — <stem>-<hash> ────────────────────────────
test_format_iis_site_name_ascii() {
    local out
    out="$(format_iis_express_site_name '/path/to/HelloApp.csproj' 'deadbeef' 2>/dev/null || true)"
    assertEquals 'ASCII stem → HelloApp-deadbeef' 'HelloApp-deadbeef' "$out"
}

test_format_iis_site_name_cjk() {
    local out
    out="$(format_iis_express_site_name '/path/中文專案.csproj' 'cafe1234' 2>/dev/null || true)"
    assertEquals 'CJK stem → 中文專案-cafe1234' '中文專案-cafe1234' "$out"
}
