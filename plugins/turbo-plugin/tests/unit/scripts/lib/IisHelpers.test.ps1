...
#
# 由於 IisHelpers.ps1 不是 main script 而是 library:本測試 dot-source 它並直接 call
...
$ScriptUnderTest = [System.IO.Path]::Combine($pluginRoot, 'scripts', 'lib', 'IisHelpers.ps1')
...
Write-Output ''
Write-Output "IisHelpers.test: Passed=$($script:Passed) Failed=$($script:Failed)"
