...
# Capability gate (U5): remove-orphan-iis.sh is a ps1-delegate (needs PowerShell + IIS Express).
# Skip cleanly on a runner without PowerShell before any fixture setup. Last line "OK" + exit 0 =
...
if ! command -v powershell >/dev/null 2>&1 && ! command -v pwsh >/dev/null 2>&1; then
    echo "OK (SKIPPED: remove-orphan-iis.sh delegates to PowerShell/IIS; no powershell/pwsh on this runner)"
    exit 0
fi
