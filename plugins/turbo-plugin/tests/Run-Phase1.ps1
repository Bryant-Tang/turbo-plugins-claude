...
#
# 用法 (從 repo root):
#   powershell -NoProfile -ExecutionPolicy Bypass -File plugins/turbo-plugin/tests/Run-Phase1.ps1
#
# 參數:
#   -RepoRoot           指定 repo 根 (預設 = 此 script 所在的上 3 層 — plugins/turbo-plugin/tests → repo root)
...
#   -TargetDoc          tracking doc 路徑 (預設 <RunDir>/phase1-results.md)
...
# Run-Phase1.ps1 lives at: <repo>/plugins/turbo-plugin/tests/Run-Phase1.ps1
...
if ([string]::IsNullOrWhiteSpace($TargetDoc)) {
    $TargetDoc = [System.IO.Path]::Combine($RunDir, 'phase1-results.md')
}
