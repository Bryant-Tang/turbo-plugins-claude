...
- **不需要 Visual Studio**:`applicationhost.config` 由第一次 `/tp-run` 依 csproj 的 `<IISUrl>` / `<IISExpressSSLPort>` / `<DevelopmentServerPort>` / `<IISExpressUseClassicPipelineMode>` 產生(也可用 `New-ApphostConfig` 單獨產),缺這個專案的站台就 append(同一份檔可放多個 web 專案、不動別人的站台);MSBuild 探測含「Build Tools for Visual Studio」,只裝 Build Tools + IIS Express 的機器也能用。
