...
param(
    ...
    [string]$BaseDir,
    [string]$DumpPath,
    # turbo-plugin-dotnet-framework-web has no SVN concern: default to skipping the SVN repo
    # build so a no-arg reset (the orchestrator's call) only mirrors the base web-app fixture.
    # Pass -SkipSvn:$false to force the (unused) SVN path.
    [switch]$SkipSvn = $true
)
