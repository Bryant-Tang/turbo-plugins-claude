#!/usr/bin/env bash
# Phase 1 — start-iis.sh (1-line delegate)
#
# Same coverage as .Tests.ps1: [iis] disabled gate consistency, missing apphost,
# missing csproj. Happy path is Phase 2 SKILL territory (real IIS Express launch).
set -uo pipefail

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd -- "$SCRIPT_DIR/../../.." && pwd)"
SCRIPT_UNDER_TEST="$REPO_ROOT/plugins/turbo-plugin/scripts/start-iis.sh"
