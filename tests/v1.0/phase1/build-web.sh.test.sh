#!/usr/bin/env bash
# Phase 1 — build-web.sh
# Note: no script-level [iis] gate (SKILL-level only); real build deferred to Phase 2.
set -uo pipefail

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd -- "$SCRIPT_DIR/../../.." && pwd)"
SCRIPT_UNDER_TEST="$REPO_ROOT/plugins/turbo-plugin/scripts/build-web.sh"
