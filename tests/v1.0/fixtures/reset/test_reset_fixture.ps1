...
#
# <repo>/tests/v1.0/fixtures/reset/test_reset_fixture.ps1
...
if (-not $dumpExists) {
    ...
    Write-Output "      Run tests/v1.0/fixtures/seed/build-seed-repo.ps1 first for full coverage."
    Write-Output ""
}
...
if (-not $dumpExists) {
    ...
    try {
        ...
        # Expected message = dict #3 entry #1 (kept in sync with build-seed-repo.ps1 / phase1-scripts.md)
