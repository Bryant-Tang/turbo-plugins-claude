...
```powershell
# 從 worktree root
.\tests\v1.0\fixtures\seed\build-seed-repo.ps1            # idempotent skip if dump exists
.\tests\v1.0\fixtures\seed\build-seed-repo.ps1 -Force     # 強制重建
```

## 25 條中文字典 mapping

build-seed-repo.ps1 取 `$zhDict.commit_messages` 前 3 條給 r5 / r10 / r15。完整 25 條字典
inline 在 `docs/test-plans/v1.0/phase1-scripts.md` 開頭 — 那是 single source of truth。
seed script 的 `$zhDict` 內容必須與 phase1-scripts.md 一致;如果改一邊請同步另一邊。
